...
select_dia="SELECT * FROM signo WHERE dia like %s"
...
select_nome = "SELECT * FROM signo WHERE nome like %s"
delete_nome = "DELETE FROM signo WHERE nome like %s"
insert = "INSERT INTO signo VALUES (%(descricao)s)"
update = '''
UPDATE signo SET
...
driver = webdriver.Chrome(executable_path='caminho do driver',chrome_options=options)
    ...
    for signo in signos:
        ...
        for row in rows:
            ...
            if signo == 'aries':
                ...
                conexao, cursor = abrir_conexao()
                cursor.execute(update, escorpiao)
                fechar_conexao(conexao)
